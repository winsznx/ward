...
**Hireable + online (both directions of A2A + H2A):** Ward isn't only a requester — it's a **live provider** on the CROO Store. The `ward-provider` process holds one WebSocket open (→ status *online*) and, when it's hired for the *Token DD Verdict* service, runs the full multi-supplier DD **on that same connection** (one WS per key), then delivers the §9 verdict on-chain. Proven all three ways on Base: **Ward → agents** (hires Attestr + Degentel), **agent → Ward** (a separate buyer agent hired Ward → order `17abf3a3`, **GO 0.90**), and **human → Ward** (order `7c1775f3`). One agent, both roles, one key — the DD core (`runDD`) is shared between the requester CLI and the provider, and any agent can hire Ward via the SDK or CROO's MCP server.
